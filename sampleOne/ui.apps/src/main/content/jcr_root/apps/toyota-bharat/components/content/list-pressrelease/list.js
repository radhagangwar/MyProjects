...
use(["/libs/wcm/foundation/components/utils/AuthoringUtils.js",
                    ...
                    ResourceUtils.getResource(pageContentResource.path + "/image")
                        .then(function (imageChildResource) {
                            ...
                            if (imageChildResource.properties["fileReference"]) {
                                var i = imageChildResource.properties["fileReference"];
								var a=i+"/jcr:content/renditions/cq5dam.thumbnail.140.100.png"
                                hasImagePromise.resolve(a);
                            } else {
    ...
    var _hasPar = function (pageResource) {
        var hasImagePromise = Q.defer();

        ResourceUtils.getResource(pageResource.path + "/jcr:content")
            .then(function (pageContentResource) {
                
                if (pageContentResource.properties["text"]) {
                    hasImagePromise.resolve(true);
                } else {
                    ResourceUtils.getResource(pageContentResource.path + "/par/newspara_text")
                        .then(function (imageChildResource) {
                            if (imageChildResource.properties["text"]) {
                               var i = imageChildResource.properties["text"];
                                 hasImagePromise.resolve(i.substring(0, 300) + "...");
                            } else {
                                ResourceUtils.getResource(imageChildResource.path + "/file")
                                    .then(function (imageFileResource) {
                                        hasImagePromise.resolve(true);
            ...
            .then(function (fullList) {
                ...
                for (var currentIndex = 0 ; currentIndex < fullList.length ; currentIndex++) {
                    ...
                    destination.push({
                        ...
                        hasPar: _hasPar(fullList[currentIndex]),
                        asd:asd
                    });
                }
...
var asd = function()  {
        ...
        var parentPath ='/content/toyota-bharat/en/pressRelease/allnews';
    	var children = parentPath.listChildren(filter);
